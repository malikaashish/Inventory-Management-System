...
public class ProductController {
    ...
    public ResponseEntity<ApiResponse<Page<ProductResponse>>> search(
        ...
        return ResponseEntity.ok(ApiResponse.success(productService.search(q, pageable)));
    }
    ...
    public ResponseEntity<ApiResponse<List<ProductResponse>>> lowStock() {
        return ResponseEntity.ok(ApiResponse.success(productService.lowStock()));
    }

    @GetMapping("/expired")
    @PreAuthorize("hasAnyRole('ADMIN','INVENTORY_STAFF')")
    public ResponseEntity<ApiResponse<List<ProductResponse>>> getExpired() {
        return ResponseEntity.ok(ApiResponse.success(productService.getExpiredProducts()));
    }
